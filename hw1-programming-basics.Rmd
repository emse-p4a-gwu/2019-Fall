...
5) Type all of your answers to the following questions in the "hw1.R" script.
...
### 4) Numeric data 1 [COLLABORATIVE, 20 pts]
...
### 5) Numeric data 2 [COLLABORATIVE, 20 pts]
...
### 6) Logical data [COLLABORATIVE, 20 pts]
...
### 7) Packages [COLLABORATIVE, 10 pts]

a) Install the package `TurtleGraphics`
b) Load the `TurtleGraphics` library then use the following command: `turtle_init()`. What happens?
c) Read the help information about the `TurtleGraphics` package:
```{r eval=FALSE}
help(package='TurtleGraphics')
```
d) Now read the help information about the `turtle_move()` function in the `TurtleGraphics` package:
```{r eval=FALSE}
?turtle_move()
```
e) Read through the following commands - write down what do you think is going to happen.
```{r eval=FALSE}
turtle_init()
...
turtle_reset()
```
f) Now run the above commands - what happened?
g) Using what you've learned about how the `TurtleGraphics` package works, write code to produce the following image:
