...
The operator `<-` is a pain to type, so RStudio has a keyboard shortcut: Alt + - (the minus sign). RStudio offers many other handy [keyboard shortcuts](https://support.rstudio.com/hc/en-us/articles/200711853-Keyboard-Shortcuts){target="_blank"}. Also, Alt+Shift+K brings up a keyboard shortcut reference card.
...
You can choose almost any name you like for an object, as long as the name does not begin with a *number* or a special character like `+`, `-`, `*`, `/`, `^`, `!`, `@`, or `&`. You will be wise to adopt a [convention for demarcating words](https://en.wikipedia.org/wiki/Camel_case){target="_blank"} in names. Here are a few:
...
Numbers in R are of the `numeric` data type, and it is the default computational type. If we assign a decimal value to a variable `x`, it will be of `numeric` type:
...
Even if we assign an integer to a variable `y`, it will still be of `numeric` type:
...
To make a number an integer, you can either add an `L` to the end, or use the `as.integer()` function to coerce it into an integer type:
...
Now that we have some non-integer and integer variables stored, we can use the `typeof()` function to distinguish between them in more detail:
...
Notice that `x` and `y` are both `double` types - that just means they are not integers. Note that you can have a decimal of `0` (e.g. `7` is the same as `7.0`, but `7L` is explicitly an integer).
...
As an example, try installing the [Wikifacts](https://github.com/keithmcnulty/wikifacts){target="_blank"} package, by Keith McNulty:
...
In case you're wondering, the only thing this package does is generate messages containing random facts from [Wikipedia](https://www.wikipedia.org/){target="_blank"}.
