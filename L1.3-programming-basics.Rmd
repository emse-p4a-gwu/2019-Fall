...
If you want to force a float number (i.e. one with decimals) into an integer value but you don't want to change the actual _type_ to an integer type, use `round()`, `ceiling()`, or `floor()`. Note that these functions produce different outcomes but all return float types (even though they look like integers):
