...
`log(x)`   | Log base e of `x` ("natural log") | `log(1)` returns ``r log(1)``
...
integer   | A number with no decimal    | `1`, `7`
...
To make a number an integer, you have to use the `as.integer()` function:
...
Notice that `x` and `y` are both `double` types - that just means they are not integers (i.e. they have decimals).
