...
#### a) `for` loop solution [5 pts]

Use a `for` loop to write the function `factorial(n)` that should return `n!`, i.e. "n factorial", which is defined for all non-negative integers. For example, `3! = 3*2*1 = 6`, `4! = 4*3*2*1 = 24`, and `5! = 5*4*3*2*1 = 120`. Note that `0` is a special case, and `0! = 1`. Assume `n >= 0`.

#### b) vector solution [5 pts]

Write the function `factorial(n)` again, but use vectors to avoid using a loop. Hint: there are some useful functions listed on the [vectors lesson page](L6-vectors.html#13_numeric_vectors) for performing operators on a vector.

### 2) `printStarTriangle(n)` [SOLO, 10 pts]
