...
1. [Download and install R](https://cloud.r-project.org){target="_blank"}.
2. [Download and install RStudio](https://www.rstudio.com/products/rstudio/download/preview/){target="_blank"}.
...
FYI: you can change the default location of the panes, among many other things: [Customizing RStudio](https://support.rstudio.com/hc/en-us/articles/200549016-Customizing-RStudio){target="_blank"}.
...
The **modulus** (aka "mod" operator) simply returns the remainder after doing regular division. For example:
...
R follows the typical BEDMAS order of operations. That is, you first calculate things inside **B**rackets, then calculate **E**xponents, then **D**ivision and **M**ultiplication, then **A**ddition and **S**ubtraction. For a more precise statement, see the [operator precedence](http://stat.ethz.ch/R-manual/R-devel/library/base/html/Syntax.html){target="_blank"} for R. For example, if I type:
...
[Using Projects](https://support.rstudio.com/hc/en-us/articles/200526207-Using-Projects){target="_blank"}
...
The mod operator (`%%`) is useful for "chopping" a number in two and returning everything to the **right** of the "cut point".

For example, `x %% 1` will return all digits to the right of the **ones place**:
...
Likewise, `x %% 10` will return all digits to the right of the **tens place**:
...
In general, you can "mod" by factors of 10 to get different results:
...
You can use integer division to do the opposite of what we just showed with the mod operator. Integer division is useful for "chopping" a number in two and returning everything to the **left** of the "cut point".

For example, `x %/% 1` will return all digits to the left of the **decimal point**:
...
Likewise, `x %/% 10` will return all digits to the left of the **ones place**:
...
Note that if the "cut point" is further to the left than the farthest-left digit, you'll get `0` back (because there's no digits beyond the cut point):

```{r}
3.1415 %/% 10
```

In general, you can integer divide by factors of 10 to get different results:
