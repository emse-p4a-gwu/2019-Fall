...
First, generate lots of random points in a square. For this example, we'll use a square with side length of 1 and centered at `(x, y) = (0, 0)`, so we need to draw random points between `x = (-0.5, 0.5)` and `y = (-0.5, 0.5)`:
...
For this example, we're putting the points in a data frame to make it easier to work with. Now that we have random x-y points, let's compute the radius to each point (the distance from zero), and then compute which points fall inside the circle, which has a radius of 0.5:
