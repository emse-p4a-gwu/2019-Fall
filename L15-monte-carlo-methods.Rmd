...
No, that's not some grainy image coming into focus - that's a Monte Carlo simulation to estimate the value of $\pi$ (`r pi`...).
...
- Guess that the Expected Odds $\tilde =$ Observed Odds = $\frac{\# Successful trials}{# Total trials}$
...
The name "Monte Carlo" comes from the (pseudo)random nature of the simulation process. Running a series of trials is similar to how many gambling games work in casinos, and Monte Carlo is known first and foremost for gambling. Of course, in the casinos, the odds are always slightly in the favor of the house, so after millions of trials, the odds are that the house will win more money than the gamblers.
...
We want to compute the integral of $f(x)$ between $3 < x < 7$. Here's what that looks like:
...
One way to estimate the shaded area is to "throw darts" at a rectangle in the x-y plane that contains the shaded area and then count how many fall below the function line. So we're going to randomly throw darts inside this box:
...
The ratio of points below the curve to the total number of points will be proportional to the ratio of the area under $f(x)$ to the total area of the rectangle. We know the total area of the rectangle is its length times its height, or $(7 - 3) = 4$ times $7^2$, which is 196. So the area under the curve should be 196 times the ratio of points under the curve to the total number of points. Let's write the code!

```{r}
# Compute if the random y value is less than or equal to f(x), which is x^2:
belowCurve <- y <= x^2
# Compute the ratio of points below the curve:
ratioBelowCurve <- sum(belowCurve) / numTrials
totalArea <- (7 - 3) * 7^2
areaUnderCurve <- ratioBelowCurve * totalArea
areaUnderCurve
```

So, using Monte Carlo simulation with 100,000 points, we estimate that the area under the curve is `r areaUnderCurve`. How'd we do?

Well, if you're familiar with calculus, the integral of $f(x) = x^2$ has a closed-form solution, so we can check our estimate against the pure-math solution:

$x^2 = \frac{x^3}{3} = \frac{7^3}{3} - \frac{3^3}{3} = 105.3333$
...
Watch this quick video to see a preview of how this works (...I hope you like lounge music)
...
So to compute $\pi$, all we need to do is multiply 4 times the ratio of the area of the circle to that of the square. Just like with integration, we can approximate that ratio by simulating lots of points and then simply counting the number that fall inside the circle.

**Let's write the code!**

First, generate lots of random points on an x and y plane between `x = (-0.5, 0.5)` and `y = (-0.5, 0.5)` - a square with side length of 1 and centered at zero:
...
Now compute the radius to each point (the distance from zero), and then compute which points fall inside the circle, which has a radius of 0.5:
...
ggplot(points) +
    geom_point(aes(x = x, y = y, color = pointInCircle), size = 0.4)
```

Looks like we correctly labeled the points! Now we have everything we need to compute $\pi$!
...
So our estimate error is: `pi - ` ``r piApprox`` `=` ``r error``.

That's an error of just `r errorPer` % - not bad for only 1000 points!

To get an even better estimate of $\pi$, increase N - here's the error with 100,000 trials:
...
ratio <- sum(points$pointInCircle) / nrow(points)
piApprox <- 4 * ratio
percentError <- round(100*abs(piApprox - pi / pi), 2)
percentError
```
