...
No, that's not some grainy image coming into focus - that's a Monte Carlo simulation approximating the value of $\pi$ (`r pi`...).
...
The ratio of points below the curve to the total number of points will be proportional to the ratio of the area under $f(x)$ to the total area of the rectangle. We know the total area of the rectangle is its length ($(7 - 3) = 4$) times its height ($7^2$), which is $4 * 49 = 196$. So the area under the curve should be 196 times the ratio of points under the curve to the total number of points. Let's write the code!
...
So, using Monte Carlo simulation with 100,000 points, we estimate that the area under the curve is `r areaUnderCurve`. How'd we do? Well, if you're familiar with calculus, the integral of $f(x) = x^2$ has a closed-form solution ($\frac{x^3}{3}$), so we can check our estimate against the pure-math solution:

$\int_{x=3}^{x=7} f(x)dx = \frac{7^3}{3} - \frac{3^3}{3} = 105.33\bar{3}$
...
As expected, by increasing N, our error decreased!
