...
### 1) `integerSquareRoot(x)` [SOLO, 10 pts]
...
### 2) `fabricYards(inches)` [SOLO, 10 pts]
...
### 3) `fabricExcess(inches)` [SOLO, 10 pts]
...
### 4) `distance(x1, y1, x2, y2)` [SOLO, 10 pts]
...
### 5) `isRightTriangle(x1, y1, x2, y2, x3, y3)` [SOLO, 10 pts]
...
### 6) `numberOfPoolBalls(rows)` [COLLABORATIVE, 10 pts]
...
### 7) `numberOfPoolBallRows(balls)` [COLLABORATIVE, 10 pts]
