...
To compare the _entire vector_, you should surround the `==` comparison within the `all()` function:

```{r}
all(x == y)
```

The `all()` function works on any types of vectors:
...
If any one element across the two vectors are different, `all()` will return `FALSE`:
