...
Instead of getting one `TRUE`, you get a vector of `TRUE`s, because the individual elements are indeed equal.

To compare the _entire vector_, you should use the `identical()` function:

```{r}
identical(x, y)
```

The `identical()` function works on any types of vectors:
...
If any one element across the two vectors are different, `identical()` will return `FALSE`:
...
One final distinction is that `identical()` also compares the _names_ of the vector elements. If you have two vectors with the same elements but different names, `identical()` will return `FALSE`. If you only care about the elements themselves (and not the names), use `all()` with a comparison:

```{r}
x <- c(one = 1, two = 2, three = 3)
y <- c(one = 1, two = 2, four = 3)
x == y
identical(x, y)
all(x == y)
```
