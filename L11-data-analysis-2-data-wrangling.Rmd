...
Before we get started, let's set up our analysis environment by following these steps:
...
2) Create a new `.R` file (File > New File > R Script), and save it as "`analysis.R`" inside your "data-analysis-tutorial" R Project folder.
...
To get started, let's load our libraries and read in the data:
...
```{r}
# Filter to view all strikes that occurred in DC
filter(df, state == 'DC')
# Filter to view all strikes that cost more than $1 million to repair
...
> Hint: One way to read the `%>%` operator is to say the words "...and then..."

For instance, in the above example I would "read" the code as "First, filter to only data from DC, **and then** select the columns `operator`, `time`, and `time_of_day`."

If you want to create a new object with this modified dataset, you can assign it a new name:
