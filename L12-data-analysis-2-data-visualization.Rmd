...
boxplot(age ~ gender,
        ...
        main = "Age distribution of male and female bear killing victims",
        xlab = 'Gender',
        ylab = 'Age')
```

# Make better figures with `ggplot2`
...
To make a histogram, we use the `geom_histogram()` layer:
...
Notice that by default `geom_histogram()` uses 30 bins, which is way too many for the month variable. Logically, we'd like to use 12 bins, so here's an improved plot:

```{r fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
ggplot(data = df) +
    geom_histogram(aes(x = month), bins = 12, fill = "darkred",
                   color = "white") +
    labs(x = "Month",
         y = "Count",
...
ggplot(data = df %>%
    ...
    labs(x = "Gender",
         y = "Age",
         title = "Age distribution of male and female bear killing victims") +
    theme_minimal()
```

## Barplots with `geom_bar()`

Bar charts are great for comparing different values of a numerical variable. By default, the `geom_bar()` layer works just like a histogram in that is plots a count of the variable provided. For example, here is the same histogram as before - the count of the bear attacks for each month in the dataset:
...
Yup, brown bears are the biggest killers, and it looks like about 1/3 are grizzlies.
