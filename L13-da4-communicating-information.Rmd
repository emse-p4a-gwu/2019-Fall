...
The goal is to integrate data analysis (executable code) with textual documentation, linking data, code, and text together in one document.

Literate programming can be directly linked to the concept of [**reproducibility**](https://en.wikipedia.org/wiki/Reproducibility). Basically, for someone to make a legitimate scientific claim, they should at minimum be able to fully reproduce their results from their raw data (and preferrably others should be able to reproduce them as well).
...
- I is clean and legible across multiple platforms (even mobile).
- All formatting is handled automatically through a browser.
- Supports raw html as well.

## Code (e.g. `R`)

Code "chunks" are defined through special notation and are executed in sequence throughout the document (just like an `.R` script). You can mix code directly in with markdown (e.g. "Here's some simple math in R: `2 + 2 = 4`") or in separate segments ("chunks") before or after markdown text. This enables you to write English language text in markdown to explain something, and then immediately follow it up with code to illustrate or demonstrate something.
...
![](images/yaml_description.png){ width=500 }
...
To make a **basic table**, use the `|` symbol to break up columns. Create a header row by adding `--------` underneath the header row:
...
Here's an example of some markdown with a code chunk on the left and the rendered output on the right:
...
There are lots of options for customizing how markdown chunks appear. You insert option immediately after the `r` in the header ` ```{r} ` and separate them with commas. For example, if you just want to display the code but don't want it to actually run, you can add `eval=FALSE`:

    `r ''````{r, eval=FALSE}
    print('hello world!')
    ```

Here's a list of other options (for more details see <http://yihui.name/knitr/>):
